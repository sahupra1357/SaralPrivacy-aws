...
const MICRO_NOTES: Record<string, string> = {
  "q2:bank_statements":
    "Bank statements reveal income, spending and counterparties — high-impact financial data that needs secure handling and a deletion rule.",
  "q2:bureau_data":
    "Credit bureau and repayment data is high-impact — restrict who can pull, view and store it.",
  "q2:aadhaar_kyc":
    "Aadhaar/KYC data should follow your regulated KYC flow, not sit in inboxes, sheets or staff phones.",
  "q3:whatsapp":
    "KYC and income documents arriving over WhatsApp are easy to forward and hard to delete or audit.",
  "q3:dsa_field_agent":
    "Documents collected by DSAs/field agents need the same controls as your core onboarding — not personal devices.",
  "q4:automated_limited_explanation":
    "Automated lending/eligibility decisions with limited explanation need a clear purpose, defined inputs and customer notice under DPDPA.",
  "q5:bundled_terms":
    "Bundling consent into terms makes it hard to show valid, traceable consent for verification, profiling and partner sharing.",
  "q6:collection_agents":
    "Sharing repayment status, contacts and references with recovery agents needs a defined purpose and monitored access.",
  "q7:personal_phone_whatsapp":
    "Agents following up on personal phones/WhatsApp puts customer financial data outside any access control.",
  "q7:download_export":
    "Letting agents export customer lists is a common and serious leakage path — restrict and log exports.",
  "q8:whatsapp":
    "Financial data in WhatsApp chats sits on personal phones, outside your core platform's access control.",
  "q8:multiple_no_sot":
    "When the same KYC/bureau/repayment data lives in sheets, WhatsApp, dashboards and devices, you can't control access or deletion.",
  "q9:indefinitely":
    "Keeping rejected applications, KYC and bank statements forever turns your data store into standing, unmanaged risk.",
  "q10:no_standard_process":
    "Without an incident process, a leaked customer list or compromised dashboard can't be contained quickly.",
};
...
export default function FintechNbfcAssessmentClient() {
  ...
  function next() {
    if (isLast) {
      ...
      setResult(r);
      setPhase("result");
      trackEvent.assessmentComplete({ score: r.readinessScore, band: r.band, industry: "fintech-nbfc" });
  ...
  function back() {
    if (qIndex === 0) router.push("/industries/fintech-nbfc");
  ...
  async function unlockReport(e: FormEvent) {
    ...
    setFormError("");
    setSubmitting(true);
    trackEvent.reportRequested({ band: result.band, sector: "fintech-nbfc" });
    ...
    const payload = JSON.stringify({
      ...
      report_type: pack.reportType, // "fintech" — single source so client + pack never drift
      city: form.city.trim(),
      answers,
      result: {
    ...
    return (
      <div className="min-h-screen bg-pearl-50">
        <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
          ...
          <div className="rounded-2xl border-2 p-7" style={{ borderColor: result.bandColor }}>
            <div className="flex flex-col items-center gap-5 sm:flex-row sm:items-center">
              ...
              <div className="text-center sm:text-left">
                <div className="text-xs font-bold uppercase tracking-wide text-slate-500">Your fintech / NBFC result</div>
          ...
          {/* Bucket map (4-tier) */}
          <div className="mt-5 rounded-xl border border-slate-200 bg-white p-6">
            <h3 className="mb-4 text-sm font-semibold text-navy-700">Your financial-data risk map</h3>
          ...
          {!reportUnlocked ? (
            <form onSubmit={unlockReport} className="mt-5 rounded-xl border border-green-200 bg-green-50 p-6">
              <div className="mb-1 flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-navy-700">Unlock your priority fixes + Fintech / NBFC DPDPA Starter Checklist</h3>
              </div>
              <p className="mb-4 text-xs text-slate-600">See your recommended next steps and get the checklist emailed to you. This scan collects no customer financial data — only your contact details and answers.</p>
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                <input aria-label="Company name" value={form.firm} onChange={(e) => setForm({ ...form, firm: e.target.value })} placeholder="Company name" className="rounded-lg border border-slate-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
          ...
          <div className="mt-5 rounded-xl bg-navy-700 p-6">
            <h3 className="text-sm font-semibold text-white">Get a Fintech / NBFC DPDPA Gap Review</h3>
            <p className="mb-4 mt-1 text-sm text-slate-300">
              We&apos;ll review your KYC flows, PAN/Aadhaar handling, bank statement collection, UPI/payment data, bureau checks, consent evidence, scoring/profiling, partner sharing, DSA/collection-agent access and customer-data retention practices.
            </p>
            <Link
              href="/contact"
              onClick={() => trackEvent.callBookingClicked({ band: result.band, location: "fintech-nbfc-assessment-result" })}
            ...
            >
              Book a Fintech / NBFC Gap Review <ArrowRight size={16} />
            </Link>
          </div>

          <FlowCrossLink sectorSlug={fintechNbfcPack.industry} source="assessment" className="mt-5" />
  ...
  return (
    <div className="min-h-screen bg-pearl-50">
      <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
        ...
        <p className="mb-5 text-xs text-slate-400">This scan collects no customer financial data — only your answers about your processes.</p>
