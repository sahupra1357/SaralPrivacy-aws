...
const MICRO_NOTES: Record<string, string> = {
  "q3:age_not_recorded":
    "If you can't tell which students are minors, you can't apply the verifiable parental-consent safeguards DPDPA requires.",
  "q4:no_specific_process":
    "With no minor-consent process, processing a child's data may lack the verifiable parental consent DPDPA requires.",
  "q4:parents_aware_not_documented":
    "Parents being 'aware' isn't the same as documented, verifiable consent you can evidence later.",
  "q5:multiple_no_standard":
    "Unstructured intake across many channels makes access, deletion and breach response much harder to control.",
  "q6:numbers_visible":
    "When parent/student numbers are visible to everyone in a group, that's exposure — not just convenience.",
  "q7:without_separate_consent":
    "Using student photos, results or testimonials — especially of minors — without separate consent is a common, serious gap.",
  "q9:share_on_request":
    "Sharing student data whenever a partner or recruiter asks, without notice or consent, is a major hidden data-flow risk.",
  "q10:indefinitely":
    "Keeping student records forever, with no policy or review, turns old data into standing risk.",
};
...
// the training-institutes assessment buckets used by the data-flow hotspots.
const BUCKET_FOCUS: Record<string, string> = {
  ...
  lms_vendor_platform: "LMS & vendor platforms",
  minor_parental_consent: "Children's data & consent",
  retention_rights: "Retention & student rights",
};

// ── Small presentational helpers ──────────────────────────────────────────────

// 4-tier bucket chip (Controlled / Moderate / High / Critical) derived from the
// raw bucket score via the engine's band thresholds. (Spec D8.)
...
export default function TrainingAssessmentClient() {
  ...
  // The marketing page (/industries/training-institutes) is the landing — open on Q1.
  ...
  // Lead capture (5 fields per spec D3)
  const [reportUnlocked, setReportUnlocked] = useState(false);
  const [form, setForm] = useState({ name: "", institute: "", email: "", phone: "", city: "", consent: false });
  ...
  function next() {
    if (isLast) {
      ...
      setResult(r);
      setPhase("result");
      trackEvent.assessmentComplete({ score: r.readinessScore, band: r.band, industry: "training-institutes" });
  ...
  function back() {
    if (qIndex === 0) router.push("/industries/training-institutes");
  ...
  async function unlockReport(e: FormEvent) {
    ...
    setFormError("");
    setSubmitting(true);
    trackEvent.reportRequested({ band: result.band, sector: "training-institutes" });
  ...
  function retake() {
    ...
    setForm({ name: "", institute: "", email: "", phone: "", city: "", consent: false });
    setFormError("");
    setSubmitting(false);
    setHpUrl("");
    setQIndex(0);
      ...
      <div className="min-h-screen bg-pearl-50">
        <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
          ...
          <div className="rounded-2xl border-2 p-7" style={{ borderColor: result.bandColor }}>
            <div className="flex flex-col items-center gap-5 sm:flex-row sm:items-center">
              ...
              <div className="text-center sm:text-left">
                <div className="text-xs font-bold uppercase tracking-wide text-slate-500">Your training institute&apos;s result</div>
          ...
          {!reportUnlocked ? (
            <form onSubmit={unlockReport} className="mt-5 rounded-xl border border-green-200 bg-green-50 p-6">
              <div className="mb-1 flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-navy-700">Unlock your priority fixes + Training Institute DPDPA Starter Checklist</h3>
              </div>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                <input aria-label="Institute name" value={form.institute} onChange={(e) => setForm({ ...form, institute: e.target.value })} placeholder="Institute name" className="rounded-lg border border-slate-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
              ...
              {typePrefill.length > 0 && (
                <p className="mt-2 text-xs text-slate-500">Institute type: {typePrefill.join(" · ")}</p>
              )}
          ...
          <div className="mt-5 rounded-xl bg-navy-700 p-6">
            <h3 className="text-sm font-semibold text-white">Get a Training Institute DPDPA Gap Review</h3>
            <p className="mb-4 mt-1 text-sm text-slate-300">
              We&apos;ll review your admission flows, parent-consent process, WhatsApp groups, LMS tools, student photos, marketing content, placement data and record-retention practices.
            </p>
            <Link
              href="/contact"
              onClick={() => trackEvent.callBookingClicked({ band: result.band, location: "training-assessment-result" })}
            ...
            >
              Book a Training Institute Gap Review <ArrowRight size={16} />
            </Link>
          </div>

          <FlowCrossLink sectorSlug={trainingInstitutePack.industry} source="assessment" className="mt-5" />
