...
const MICRO_NOTES: Record<string, string> = {
  "q3:aadhaar":
    "Aadhaar copies are high-impact — collect only if needed, mask where possible, and don't keep them on staff phones.",
  "q3:bank_loan_docs":
    "Bank statements and loan documents are sensitive financial data — they need secure handling and a deletion rule.",
  "q3:passport_nri":
    "Passport and NRI documents are high-impact identity records — control who can access and forward them.",
  "q4:whatsapp":
    "KYC and property papers arriving over WhatsApp are easy to forward and hard to delete consistently.",
  ...
  "q5:whatsapp":
    "Storing leads and KYC in WhatsApp chats means client data sits on personal phones, outside any access control.",
  "q5:multiple_no_sot":
    "When the same lead/KYC lives in sheets, WhatsApp, phones and cloud, you can't control access, leakage or deletion.",
  "q6:whatsapp_broker_groups":
    "Forwarding buyer/tenant details into co-broker WhatsApp groups exposes them well beyond the intended deal.",
  "q6:loan_partners":
    "Sharing income and loan documents with lenders should be purposeful, consented and access-controlled.",
  "q7:ex_staff_partner_access":
    "Ex-staff or old broker partners retaining access to lead sheets or CRM is a common and serious control gap.",
  "q8:no_documentation":
    "Without clear lead/site-visit form language, clients aren't told their data goes to brokers, builders and lenders.",
  "q9:indefinitely":
    "Keeping old leads and KYC forever for 'future deals' turns your database into standing, unmanaged risk.",
  "q10:no_standard_process":
    "Without an incident process, a wrong-recipient WhatsApp or forwarded PAN copy can't be contained quickly.",
};
...
export default function RealEstateAssessmentClient() {
  ...
  function next() {
    if (isLast) {
      ...
      setResult(r);
      setPhase("result");
      trackEvent.assessmentComplete({ score: r.readinessScore, band: r.band, industry: "real-estate" });
  ...
  function back() {
    if (qIndex === 0) router.push("/industries/real-estate");
  ...
  async function unlockReport(e: FormEvent) {
    ...
    setFormError("");
    setSubmitting(true);
    trackEvent.reportRequested({ band: result.band, sector: "real-estate" });
    ...
    const payload = JSON.stringify({
      ...
      report_type: pack.reportType, // "realty" — single source so client + pack never drift
      city: form.city.trim(),
      answers,
      result: {
      ...
      <div className="min-h-screen bg-pearl-50">
        <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
          ...
          {!reportUnlocked ? (
            <form onSubmit={unlockReport} className="mt-5 rounded-xl border border-green-200 bg-green-50 p-6">
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                <input aria-label="Firm or agency name" value={form.firm} onChange={(e) => setForm({ ...form, firm: e.target.value })} placeholder="Firm / agency name" className="rounded-lg border border-slate-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
          ...
          <div className="mt-5 rounded-xl bg-navy-700 p-6">
            ...
            <Link
              href="/contact"
              onClick={() => trackEvent.callBookingClicked({ band: result.band, location: "real-estate-assessment-result" })}
            ...
            >
              Book a Real Estate Gap Review <ArrowRight size={16} />
            </Link>
          </div>

          <FlowCrossLink sectorSlug={realEstatePack.industry} source="assessment" className="mt-5" />
