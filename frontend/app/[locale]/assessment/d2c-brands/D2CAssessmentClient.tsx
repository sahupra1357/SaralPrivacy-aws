...
const pack = d2cBrandsPack;
const BAND_COPY = pack.bandCopy!;

// ── Contextual micro-feedback (UI layer) ─────────────────────────────────────
const MICRO_NOTES: Record<string, string> = {
  "q2:health_body":
    "Skin, hair, body or wellness data is sensitive — it needs stronger consent, purpose limits and access controls.",
  "q2:marketplace_exports":
    "Reusing customer data exported from marketplaces for your own marketing usually has no consent behind it.",
  "q3:multiple_no_preference":
    "Many channels with no central preference make it hard to honour a customer's opt-out everywhere.",
  "q4:send_until_complain":
    "Messaging people until they complain is marketing without consent — the opposite of DPDPA's opt-in rule.",
  "q4:checkbox_prechecked":
    "A pre-ticked box isn't a clear opt-in — consent must be a deliberate choice.",
  "q5:no_clear_process":
    "With no opt-out process, customers can't withdraw consent — a core DPDPA gap.",
  "q6:segmentation_lookalike":
    "Segmentation and lookalike audiences profile customers and share data with ad platforms — this needs disclosure and consent.",
  "q7:agency_scripts":
    "Tracking you can't account for is the riskiest kind — you can't disclose or control what you can't see.",
  "q9:ex_staff_agency_access":
    "Ex-staff or agency access to customer data that's never revoked is a common, serious control gap.",
  "q10:keep_forever":
    "Keeping all customer data forever turns your database into standing, unmanaged risk.",
};
...
// d2c-brands assessment buckets used by the data-flow hotspots.
const BUCKET_FOCUS: Record<string, string> = {
  customer_data_collection: "Customer data collection",
  marketing_consent: "Marketing consent",
  tracking_adtech: "Tracking & adtech",
  vendor_fulfilment: "Vendors & fulfilment",
  retention_preferences: "Retention & preferences",
};
...
export default function D2CAssessmentClient() {
  ...
  const [form, setForm] = useState({ name: "", brand: "", email: "", phone: "", city: "", consent: false });
  ...
  function next() {
    if (isLast) {
      ...
      setResult(r);
      setPhase("result");
      trackEvent.assessmentComplete({ score: r.readinessScore, band: r.band, industry: "d2c-brands" });
  ...
  function back() {
    if (qIndex === 0) router.push("/industries/d2c-brands");
  ...
  async function unlockReport(e: FormEvent) {
    ...
    setFormError("");
    setSubmitting(true);
    trackEvent.reportRequested({ band: result.band, sector: "d2c-brands" });

    const payload = JSON.stringify({
      email: form.email.trim(),
      name: form.name.trim(),
      business: form.brand.trim(),
      mobile: form.phone.trim(),
      industry: "d2c-brands",
      report_type: pack.reportType, // "d2c" — single source so client + pack never drift
      city: form.city.trim(),
      answers,
      result: {
  ...
  function retake() {
    ...
    setForm({ name: "", brand: "", email: "", phone: "", city: "", consent: false });
    setFormError("");
    setSubmitting(false);
    setHpUrl("");
    setQIndex(0);
    ...
    return (
      <div className="min-h-screen bg-pearl-50">
        <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
          ...
          <div className="rounded-2xl border-2 p-7" style={{ borderColor: result.bandColor }}>
            <div className="flex flex-col items-center gap-5 sm:flex-row sm:items-center">
              ...
              <div className="text-center sm:text-left">
                <div className="text-xs font-bold uppercase tracking-wide text-slate-500">Your D2C brand&apos;s result</div>
          ...
          {/* Bucket map (4-tier) */}
          <div className="mt-5 rounded-xl border border-slate-200 bg-white p-6">
            <h3 className="mb-4 text-sm font-semibold text-navy-700">Your customer-data risk map</h3>
          ...
          {!reportUnlocked ? (
            <form onSubmit={unlockReport} className="mt-5 rounded-xl border border-green-200 bg-green-50 p-6">
              <div className="mb-1 flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-navy-700">Unlock your priority fixes + D2C Brand DPDPA Starter Checklist</h3>
              </div>
              ...
              {channelPrefill.length > 0 && (
                <p className="mt-2 text-xs text-slate-500">Store profile: {channelPrefill.join(" · ")}</p>
              )}
          ...
          <div className="mt-5 rounded-xl bg-navy-700 p-6">
            <h3 className="text-sm font-semibold text-white">Get a D2C Brand DPDPA Gap Review</h3>
            <p className="mb-4 mt-1 text-sm text-slate-300">
              We&apos;ll review your marketing consent, WhatsApp/SMS/email opt-in, tracking pixels, cart and lifecycle flows, vendor sharing, store-admin access and customer-data retention.
            </p>
            <Link
              href="/contact"
              onClick={() => trackEvent.callBookingClicked({ band: result.band, location: "d2c-assessment-result" })}
