...
const pack = lawFirmsPack;
const BAND_COPY = pack.bandCopy!;

// ── Contextual micro-feedback (UI layer) ─────────────────────────────────────
const MICRO_NOTES: Record<string, string> = {
  "q2:evidence_files":
    "Evidence files, screenshots, call records and videos are high-sensitivity — they need restricted access and a clear retention rule.",
  "q2:sensitive_records":
    "Medical, family, criminal and disciplinary records are among the most sensitive personal data your firm holds.",
  "q3:whatsapp":
    "Client documents arriving over WhatsApp are hard to govern, track and delete consistently.",
  "q3:informal_evidence_exports":
    "Screenshots, audio/video and chat exports sent informally are easy to lose track of and hard to secure.",
  "q4:whatsapp":
    "Storing matter files in WhatsApp chats means client data sits on personal phones, outside any access control.",
  "q4:multiple_no_sot":
    "When the same matter file lives in email, laptops, cloud and WhatsApp, you can't control access, leakage or deletion.",
  "q5:ex_staff_access":
    "Ex-staff or former interns retaining access to client files is one of the most common and serious control gaps.",
  "q5:most_staff_access":
    "When most staff can open most client files, a single matter's confidentiality depends on everyone, not the matter team.",
  "q6:no_documentation":
    "Without clear engagement-letter language, clients aren't told how their data is used, shared, stored and retained.",
  "q7:documented_controlled":
    "Documented purpose and controlled access genuinely lower your sharing risk — keep it current and reviewed.",
  "q8:no_classification":
    "Not classifying sensitive matters means family, criminal and employment files sit alongside everything else.",
  "q8:stored_regular_files":
    "Storing sensitive matters with regular files removes the extra protection those matters need.",
  "q9:indefinitely":
    "Keeping every closed matter forever turns your archive into standing, unmanaged risk.",
  "q10:no_standard_process":
    "Without an incident process, a wrong-recipient email or exposed cloud folder can't be contained quickly.",
};
...
// law-firms assessment buckets used by the data-flow hotspots - ALL FIVE must be
// present or the deep-link guard test fails, because a hotspot can send any of
// them and a missing key renders no banner while nothing errors.
const BUCKET_FOCUS: Record<string, string> = {
  client_matter_data: "Client & matter data",
  case_evidence_sensitivity: "Evidence & sensitive matters",
  document_sharing_court_workflow: "Document sharing & court filing",
  staff_junior_vendor_access: "Staff, junior & vendor access",
...
export default function LawFirmAssessmentClient() {
  ...
  function next() {
    if (isLast) {
      ...
      setResult(r);
      setPhase("result");
      trackEvent.assessmentComplete({ score: r.readinessScore, band: r.band, industry: "law-firms" });
  ...
  function back() {
    if (qIndex === 0) router.push("/industries/law-firms");
  ...
  async function unlockReport(e: FormEvent) {
    ...
    setFormError("");
    setSubmitting(true);
    trackEvent.reportRequested({ band: result.band, sector: "law-firms" });
    ...
    const payload = JSON.stringify({
      ...
      industry: "law-firms",
      report_type: pack.reportType, // "law-firm" — single source so client + pack never drift
      city: form.city.trim(),
      answers,
      result: {
    ...
    return (
      <div className="min-h-screen bg-pearl-50">
        <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
          ...
          {/* Bucket map (4-tier) */}
          <div className="mt-5 rounded-xl border border-slate-200 bg-white p-6">
            <h3 className="mb-4 text-sm font-semibold text-navy-700">Your client &amp; matter-data risk map</h3>
          ...
          {!reportUnlocked ? (
            <form onSubmit={unlockReport} className="mt-5 rounded-xl border border-green-200 bg-green-50 p-6">
              <div className="mb-1 flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-navy-700">Unlock your priority fixes + Law Firm DPDPA Starter Checklist</h3>
              </div>
          ...
          <div className="mt-5 rounded-xl bg-navy-700 p-6">
            <h3 className="text-sm font-semibold text-white">Get a Law Firm DPDPA Gap Review</h3>
            <p className="mb-4 mt-1 text-sm text-slate-300">
              We&apos;ll review your matter intake, client KYC handling, case-file storage, sensitive-matter access, junior/intern permissions, court/vendor sharing, cloud folders, WhatsApp/email workflows and closed-file retention.
            </p>
            <Link
              href="/contact"
              onClick={() => trackEvent.callBookingClicked({ band: result.band, location: "law-firm-assessment-result" })}
  ...
  return (
    <div className="min-h-screen bg-pearl-50">
      <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
        ...
        <p className="mb-5 text-xs text-slate-400">This scan collects no client documents — only your answers about your processes.</p>
                ...
                <div key={opt.id}>
                  <label className="block cursor-pointer">
                    ...
                    <div className={cn(
                      ...
                      // modifier that COMBINES with the parties above, not a standalone answer.
                      ...
                      <span className="flex-1">
                        ...
                        {opt.riskPoints < 0 && (
                          <span className="mt-0.5 block text-xs font-normal text-green-700">
                            Lowers this section&apos;s risk — tick this in addition to the parties above if it applies.
                          </span>
                        )}
                      </span>
                      {opt.badge && (
