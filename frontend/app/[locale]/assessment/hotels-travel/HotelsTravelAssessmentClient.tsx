...
const pack = hotelsTravelPack;
// Short focus labels for the Data Flow Map deep-link (?bucket=). Keys match the
// hotels-travel assessment buckets used by the data-flow hotspots - every one of
// pack.assessmentBuckets must appear here or the deep-link guard test fails.
const BUCKET_FOCUS: Record<string, string> = {
  guest_traveller_data: "Guest & traveller data",
  id_passport_travel_document: "ID, passport & travel documents",
  booking_ota_vendor_sharing: "Booking, OTA & vendor sharing",
  system_staff_access: "System, staff & access",
  retention_marketing_incident: "Retention & incident readiness",
};
const BAND_COPY = pack.bandCopy!;

// ── Contextual micro-feedback (UI layer) ─────────────────────────────────────
const MICRO_NOTES: Record<string, string> = {
  "q2:health_accessibility":
    "Health, accessibility or special-assistance details are high-impact — collect only what you need and limit who can see them.",
  "q3:passport":
    "Passport copies are high-impact identity records — control who can access and forward them, and don't keep them on staff phones.",
  "q3:visa":
    "Visa documents are sensitive — store them securely and delete them once the booking or trip is complete.",
  "q3:foreign_guest_cform":
    "Foreign guest / C-Form data should follow a controlled process and a clear retention rule, not sit in inboxes forever.",
  "q4:whatsapp":
    "Passport copies and IDs arriving over WhatsApp are easy to forward and hard to delete consistently.",
  ...
  "q5:whatsapp":
    "Storing guest IDs in WhatsApp chats means guest data sits on personal phones, outside any access control.",
  "q5:multiple_no_sot":
    "When the same guest record lives in PMS, OTA dashboards, WhatsApp, phones and email, you can't control access or deletion.",
  "q6:visa_consultant":
    "Sharing passport/visa data with documentation consultants should be purposeful, consented and access-controlled.",
  "q7:shared_logins":
    "Shared front-desk or PMS logins make it impossible to know who accessed which guest record.",
  "q7:ex_staff_vendor_access":
    "Ex-staff or old vendor/agent access to PMS, OTA dashboards or WhatsApp is a common and serious control gap.",
  "q8:cctv_sensitive":
    "CCTV in lifts, parking and entry/exit points needs a documented purpose, access control and retention rule.",
  "q9:indefinitely":
    "Keeping old guest IDs and passport copies forever turns your records into standing, unmanaged risk.",
  "q10:no_standard_process":
    "Without an incident process, a wrong-recipient WhatsApp or exposed passport folder can't be contained quickly.",
};
...
export default function HotelsTravelAssessmentClient() {
  ...
  function next() {
    if (isLast) {
      ...
      setResult(r);
      setPhase("result");
      trackEvent.assessmentComplete({ score: r.readinessScore, band: r.band, industry: "hotels-travel" });
  ...
  function back() {
    if (qIndex === 0) router.push("/industries/hotels-travel");
  ...
  async function unlockReport(e: FormEvent) {
    ...
    setFormError("");
    setSubmitting(true);
    trackEvent.reportRequested({ band: result.band, sector: "hotels-travel" });
    ...
    const payload = JSON.stringify({
      ...
      report_type: pack.reportType, // "hotel" — single source so client + pack never drift
      city: form.city.trim(),
      answers,
      result: {
    ...
    return (
      <div className="min-h-screen bg-pearl-50">
        <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
          ...
          <div className="rounded-2xl border-2 p-7" style={{ borderColor: result.bandColor }}>
            <div className="flex flex-col items-center gap-5 sm:flex-row sm:items-center">
              ...
              <div className="text-center sm:text-left">
                <div className="text-xs font-bold uppercase tracking-wide text-slate-500">Your hotel &amp; travel result</div>
          ...
          {/* Bucket map (4-tier) */}
          <div className="mt-5 rounded-xl border border-slate-200 bg-white p-6">
            <h3 className="mb-4 text-sm font-semibold text-navy-700">Your guest &amp; travel-data risk map</h3>
          ...
          {!reportUnlocked ? (
            <form onSubmit={unlockReport} className="mt-5 rounded-xl border border-green-200 bg-green-50 p-6">
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                <input aria-label="Hotel or business name" value={form.firm} onChange={(e) => setForm({ ...form, firm: e.target.value })} placeholder="Hotel / business name" className="rounded-lg border border-slate-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
          ...
          <div className="mt-5 rounded-xl bg-navy-700 p-6">
            <h3 className="text-sm font-semibold text-white">Get a Hotels &amp; Travel DPDPA Gap Review</h3>
            <p className="mb-4 mt-1 text-sm text-slate-300">
              We&apos;ll review your guest-data flows, ID/passport handling, OTA sharing, booking systems, WhatsApp/email workflows, travel-document processing, vendor access, CCTV practices and old record-retention controls.
            </p>
            <Link
              href="/contact"
              onClick={() => trackEvent.callBookingClicked({ band: result.band, location: "hotels-travel-assessment-result" })}
  ...
  return (
    <div className="min-h-screen bg-pearl-50">
      <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
        ...
        <p className="mb-5 text-xs text-slate-400">This scan collects no guest documents — only your answers about your processes.</p>
