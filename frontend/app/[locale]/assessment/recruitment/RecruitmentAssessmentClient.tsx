...
const pack = recruitmentAgenciesPack;
const BAND_COPY = pack.bandCopy!;

// ── Contextual micro-feedback (UI layer) ─────────────────────────────────────
const MICRO_NOTES: Record<string, string> = {
  "q2:scraped_profiles":
    "Scraped or public-search profiles are high exposure unless candidates are clearly informed how their data is used.",
  "q2:multiple_no_sot":
    "Profiles from many channels with no single source of truth make access, deletion and notice hard to control.",
  "q4:initial_registration":
    "Collecting PAN/Aadhaar/salary/BGV at registration — before they're needed — is excessive-collection risk.",
  "q4:whenever_shared":
    "Accepting high-impact documents whenever candidates send them invites data you don't yet need.",
  "q5:no_documented_notice":
    "Without a documented notice, candidates don't know their profile will be stored and shared — a core DPDPA gap.",
  "q6:bulk_cv_folders":
    "Bulk CV folders shared with many client users are hard to recall, restrict or delete.",
  "q6:client_ats_access":
    "Giving clients access to your candidate database extends exposure well beyond the role they're hiring for.",
  "q7:ex_staff_access":
    "Ex-recruiter access to candidate data is one of the most common and serious control gaps.",
  "q9:indefinitely":
    "Keeping rejected/inactive profiles forever turns your database into standing, unmanaged risk.",
};
...
// Short focus labels for the Data Flow Map deep-link (?bucket=). Keys match
// the pack bucket keys; a foreign value is ignored (no banner).
const BUCKET_FOCUS: Record<string, string> = {
  candidate_sourcing: "Candidate sourcing",
  candidate_document: "Candidate documents",
  client_sharing: "Client sharing",
  ats_tool_access: "Where candidate data sits",
  retention_rights: "Retention & candidate rights",
};

export default function RecruitmentAssessmentClient() {
  ...
  const [form, setForm] = useState({ name: "", agency: "", email: "", phone: "", city: "", consent: false });
  ...
  function next() {
    if (isLast) {
      ...
      setResult(r);
      setPhase("result");
      trackEvent.assessmentComplete({ score: r.readinessScore, band: r.band, industry: "recruitment-agencies" });
  ...
  function back() {
    if (qIndex === 0) router.push("/industries/recruitment-agencies");
  ...
  async function unlockReport(e: FormEvent) {
    ...
    setFormError("");
    setSubmitting(true);
    trackEvent.reportRequested({ band: result.band, sector: "recruitment-agencies" });

    const payload = JSON.stringify({
      email: form.email.trim(),
      name: form.name.trim(),
      business: form.agency.trim(),
      mobile: form.phone.trim(),
      industry: "recruitment-agencies",
      report_type: pack.reportType, // "recruit" — single source so client + pack never drift
      city: form.city.trim(),
      answers,
      result: {
  ...
  function retake() {
    ...
    setForm({ name: "", agency: "", email: "", phone: "", city: "", consent: false });
    setFormError("");
    setSubmitting(false);
    setHpUrl("");
    setQIndex(0);
    ...
    return (
      <div className="min-h-screen bg-pearl-50">
        <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
          ...
          <div className="rounded-2xl border-2 p-7" style={{ borderColor: result.bandColor }}>
            <div className="flex flex-col items-center gap-5 sm:flex-row sm:items-center">
              ...
              <div className="text-center sm:text-left">
                <div className="text-xs font-bold uppercase tracking-wide text-slate-500">Your recruitment agency&apos;s result</div>
          ...
          {/* Bucket map (4-tier) */}
          <div className="mt-5 rounded-xl border border-slate-200 bg-white p-6">
            <h3 className="mb-4 text-sm font-semibold text-navy-700">Your candidate-data risk map</h3>
          ...
          {!reportUnlocked ? (
            <form onSubmit={unlockReport} className="mt-5 rounded-xl border border-green-200 bg-green-50 p-6">
              <div className="mb-1 flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-navy-700">Unlock your priority fixes + Recruitment Agency DPDPA Starter Checklist</h3>
              </div>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                <input aria-label="Agency name" value={form.agency} onChange={(e) => setForm({ ...form, agency: e.target.value })} placeholder="Agency name" className="rounded-lg border border-slate-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
              ...
              {typePrefill.length > 0 && (
                <p className="mt-2 text-xs text-slate-500">Agency type: {typePrefill.join(" · ")}</p>
              )}
          ...
          <div className="mt-5 rounded-xl bg-navy-700 p-6">
            <h3 className="text-sm font-semibold text-white">Get a Recruitment Agency DPDPA Gap Review</h3>
            <p className="mb-4 mt-1 text-sm text-slate-300">
              We&apos;ll review your candidate sourcing, CV sharing, ATS access, WhatsApp/email workflows, client sharing, background-verification data, recruiter access and rejected-candidate retention practices.
            </p>
            <Link
              href="/contact"
              onClick={() => trackEvent.callBookingClicked({ band: result.band, location: "recruitment-assessment-result" })}
            ...
            >
              Book a Recruitment Agency Gap Review <ArrowRight size={16} />
            </Link>
          </div>
  ...
  return (
    <div className="min-h-screen bg-pearl-50">
      <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
        ...
        {/* Data Flow Map deep-link context (addendum §H1) */}
        {focusLabel && (
