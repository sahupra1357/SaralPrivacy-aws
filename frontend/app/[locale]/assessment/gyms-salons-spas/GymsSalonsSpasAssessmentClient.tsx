...
const pack = gymsSalonsSpasPack;
const BAND_COPY = pack.bandCopy!;

// Deep-link focus from the Data Flow Map hotspots
// (/industries/gyms-salons-spas/data-flow -> ?from=data-flow&bucket=<key>).
// Must cover EVERY key in gymsSalonsSpasDataFlowPack.assessmentBuckets, or the
// link lands, the banner silently never renders, and nothing errors.
// Test-enforced by the "hotspot deep-links resolve" guard test.
const BUCKET_FOCUS: Record<string, string> = {
  customer_membership_data: "Customer & membership data",
  health_body_consultation_data: "Health, body & consultation data",
  photos_marketing_whatsapp: "Photos, marketing & WhatsApp",
  app_staff_vendor_access: "App, staff & vendor access",
  retention_rights_incident: "Retention & incident readiness",
};

// ── Contextual micro-feedback (UI layer) ─────────────────────────────────────
const MICRO_NOTES: Record<string, string> = {
  "q2:health_declaration":
    "Medical conditions, injuries and allergies are high-impact health data — limit who can see them and don't keep them on staff phones.",
  "q2:fitness_body_data":
    "Weight, BMI and body measurements are high-impact — collect only what's needed and restrict access.",
  "q2:photos_videos":
    "Customer photos and before-after images are personal data — using them needs consent and a removal route.",
  "q3:apps_whatsapp_notes_no_control":
    "Health and body notes in apps, WhatsApp or staff notes without access control are a common, avoidable exposure.",
  "q4:whatsapp":
    "Health notes and photos arriving over WhatsApp are easy to forward and hard to delete consistently.",
  "q4:personal_phone_photos":
    "Customer photos on staff personal phones are hard to track, secure and delete.",
  "q5:no_separate_consent":
    "Using customer photos or testimonials for ads without separate consent is a frequent DPDPA exposure — get consent and offer removal.",
  "q6:whatsapp_promotions":
    "Promotional WhatsApp needs a clear opt-out, separate from appointment reminders.",
  "q7:fitness_biometric_device":
    "Body-analysis and biometric devices process health/body data — keep the purpose clear and access restricted.",
  "q8:personal_phone_whatsapp":
    "Staff using personal phones/WhatsApp for customers puts photos and health notes outside any control.",
  "q8:shared_logins":
    "Shared appointment-app or CRM logins make it impossible to know who accessed which customer record.",
  "q8:ex_staff_vendor_access":
    "Ex-staff or old vendor access to customer lists and photos is a common and serious control gap.",
  "q9:indefinitely":
    "Keeping old photos, consultation notes and body metrics forever turns your records into standing, unmanaged risk.",
  "q10:no_standard_process":
    "Without an incident process, a wrong-photo share or leaked health note can't be contained quickly.",
};
...
export default function GymsSalonsSpasAssessmentClient() {
  ...
  function next() {
    if (isLast) {
      ...
      setResult(r);
      setPhase("result");
      trackEvent.assessmentComplete({ score: r.readinessScore, band: r.band, industry: "gyms-salons-spas" });
  ...
  function back() {
    if (qIndex === 0) router.push("/industries/gyms-salons-spas");
  ...
  async function unlockReport(e: FormEvent) {
    ...
    setFormError("");
    setSubmitting(true);
    trackEvent.reportRequested({ band: result.band, sector: "gyms-salons-spas" });
    ...
    const payload = JSON.stringify({
      ...
      report_type: pack.reportType, // "wellness" — single source so client + pack never drift
      city: form.city.trim(),
      answers,
      result: {
    ...
    return (
      <div className="min-h-screen bg-pearl-50">
        <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
          ...
          <div className="rounded-2xl border-2 p-7" style={{ borderColor: result.bandColor }}>
            <div className="flex flex-col items-center gap-5 sm:flex-row sm:items-center">
              ...
              <div className="text-center sm:text-left">
                <div className="text-xs font-bold uppercase tracking-wide text-slate-500">Your gym / salon / spa result</div>
          ...
          {/* Bucket map (4-tier) */}
          <div className="mt-5 rounded-xl border border-slate-200 bg-white p-6">
            <h3 className="mb-4 text-sm font-semibold text-navy-700">Your customer &amp; wellness-data risk map</h3>
          ...
          {!reportUnlocked ? (
            <form onSubmit={unlockReport} className="mt-5 rounded-xl border border-green-200 bg-green-50 p-6">
              <div className="mb-1 flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-navy-700">Unlock your priority fixes + Gym / Salon / Spa DPDPA Starter Checklist</h3>
              </div>
              <p className="mb-4 text-xs text-slate-600">See your recommended next steps and get the checklist emailed to you. This scan collects no customer photos or health notes — only your contact details and answers.</p>
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                <input aria-label="Business name" value={form.firm} onChange={(e) => setForm({ ...form, firm: e.target.value })} placeholder="Business name" className="rounded-lg border border-slate-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
          ...
          <div className="mt-5 rounded-xl bg-navy-700 p-6">
            <h3 className="text-sm font-semibold text-white">Get a Gym / Salon / Spa DPDPA Gap Review</h3>
            <p className="mb-4 mt-1 text-sm text-slate-300">
              We&apos;ll review your membership forms, appointment workflows, health/body data, customer photos, WhatsApp campaigns, staff access, appointment apps, vendor platforms and old customer-record retention practices.
            </p>
            <Link
              href="/contact"
              onClick={() => trackEvent.callBookingClicked({ band: result.band, location: "gyms-salons-spas-assessment-result" })}
  ...
  return (
    <div className="min-h-screen bg-pearl-50">
      <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
        ...
        <p className="mb-5 text-xs text-slate-400">This scan collects no customer photos, health notes or records — only your answers about your processes.</p>
