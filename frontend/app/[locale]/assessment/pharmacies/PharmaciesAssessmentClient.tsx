...
// pharmacies assessment buckets used by the data-flow hotspots - every one of
// pack.assessmentBuckets must appear here or the deep-link guard test fails.
const BUCKET_FOCUS: Record<string, string> = {
  customer_prescription_data: "Customer & prescription data",
  health_indicator_medicine_history: "Health indicators & medicine history",
  order_delivery_vendor_sharing: "Order, delivery & vendor sharing",
  system_staff_access: "System, staff & access",
  retention_refill_incident: "Retention, refill & incident readiness",
};
const BAND_COPY = pack.bandCopy!;

// ── Contextual micro-feedback (UI layer) ─────────────────────────────────────
const MICRO_NOTES: Record<string, string> = {
  "q2:prescription_image":
    "Prescription images are high-impact — keep them in a secure system, not on staff phones, and delete them on a schedule.",
  "q2:medicine_history":
    "Medicine order history can reveal health conditions even without a diagnosis field — treat it as high-impact.",
  "q2:diagnosis_notes":
    "Health condition / diagnosis notes should be limited to staff who genuinely need them, never broad counter access.",
  "q3:mental_health":
    "Mental-health medicine data is high-impact — avoid using it for reminders or offers without clear, separate consent.",
  "q3:hiv_infectious":
    "HIV / infectious-disease medicine data is high-impact — strict access control and no marketing reuse.",
  "q4:whatsapp":
    "Prescription images arriving over WhatsApp are easy to forward and hard to delete consistently.",
  "q4:multiple_no_standard":
    "Many intake channels with no standard process make access, forwarding, retention and deletion hard to control.",
  "q5:whatsapp":
    "Storing prescriptions in WhatsApp chats means health-linked data sits on personal phones, outside any access control.",
  "q5:staff_devices":
    "Prescription images on staff phones or laptops are hard to track, secure and delete.",
  "q5:multiple_no_sot":
    "When the same prescription lives in billing software, WhatsApp, phones and sheets, you can't control access or deletion.",
  "q6:delivery_partner":
    "Delivery partners should see only what they need for fulfilment — not full prescription or medicine-history details.",
  "q7:based_on_history_no_consent":
    "Refill messages based on medicine history need clear opt-in, unsubscribe and preference controls.",
  "q8:shared_logins":
    "Shared billing/POS logins make it impossible to know who accessed which prescription record.",
  "q8:delivery_over_access":
    "Delivery staff or vendors viewing full prescription details beyond delivery need is a serious control gap.",
  "q9:indefinitely":
    "Keeping old prescriptions and medicine history forever turns your records into standing, unmanaged risk.",
  "q10:no_standard_process":
    "Without an incident process, a wrong-prescription WhatsApp or exposed order can't be contained quickly.",
};
...
export default function PharmaciesAssessmentClient() {
  ...
  // Data Flow Map hotspots deep-link here with ?bucket=, so the scan opens
  // acknowledging the risk area the reader tapped. Requires the <Suspense>
  // boundary in page.tsx, or `next build` fails.
  ...
  const [form, setForm] = useState({ name: "", firm: "", email: "", phone: "", city: "", consent: false });
  ...
  function next() {
    if (isLast) {
      ...
      setResult(r);
      setPhase("result");
      trackEvent.assessmentComplete({ score: r.readinessScore, band: r.band, industry: "pharmacies" });
  ...
  function back() {
    if (qIndex === 0) router.push("/industries/pharmacies");
  ...
  async function unlockReport(e: FormEvent) {
    ...
    setFormError("");
    setSubmitting(true);
    trackEvent.reportRequested({ band: result.band, sector: "pharmacies" });
    ...
    const payload = JSON.stringify({
      ...
      industry: "pharmacies",
      report_type: pack.reportType, // "pharmacy" — single source so client + pack never drift
      city: form.city.trim(),
      answers,
      result: {
  ...
  function retake() {
    ...
    setForm({ name: "", firm: "", email: "", phone: "", city: "", consent: false });
    setFormError("");
    setSubmitting(false);
    setHpUrl("");
    setQIndex(0);
    ...
    return (
      <div className="min-h-screen bg-pearl-50">
        <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
          ...
          <div className="rounded-2xl border-2 p-7" style={{ borderColor: result.bandColor }}>
            <div className="flex flex-col items-center gap-5 sm:flex-row sm:items-center">
              ...
              <div className="text-center sm:text-left">
                <div className="text-xs font-bold uppercase tracking-wide text-slate-500">Your pharmacy result</div>
          ...
          {/* Bucket map (4-tier) */}
          <div className="mt-5 rounded-xl border border-slate-200 bg-white p-6">
            <h3 className="mb-4 text-sm font-semibold text-navy-700">Your prescription &amp; medicine-data risk map</h3>
          ...
          {!reportUnlocked ? (
            <form onSubmit={unlockReport} className="mt-5 rounded-xl border border-green-200 bg-green-50 p-6">
              <div className="mb-1 flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-navy-700">Unlock your priority fixes + Pharmacy DPDPA Starter Checklist</h3>
              </div>
              <p className="mb-4 text-xs text-slate-600">See your recommended next steps and get the checklist emailed to you. This scan collects no prescriptions or patient records — only your contact details and answers.</p>
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                <input aria-label="Pharmacy or business name" value={form.firm} onChange={(e) => setForm({ ...form, firm: e.target.value })} placeholder="Pharmacy / business name" className="rounded-lg border border-slate-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
              ...
              {typePrefill.length > 0 && (
                <p className="mt-2 text-xs text-slate-500">Business type: {typePrefill.join(" · ")}</p>
              )}
          ...
          <div className="mt-5 rounded-xl bg-navy-700 p-6">
            <h3 className="text-sm font-semibold text-white">Get a Pharmacy DPDPA Gap Review</h3>
            <p className="mb-4 mt-1 text-sm text-slate-300">
              We&apos;ll review your prescription flows, WhatsApp orders, medicine-history handling, refill reminders, delivery partner sharing, billing software access, branch controls and old prescription-retention practices.
            </p>
            <Link
              href="/contact"
              onClick={() => trackEvent.callBookingClicked({ band: result.band, location: "pharmacies-assessment-result" })}
  ...
  return (
    <div className="min-h-screen bg-pearl-50">
      <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
        ...
        <p className="mb-5 text-xs text-slate-400">This scan collects no prescriptions or patient records — only your answers about your processes.</p>
